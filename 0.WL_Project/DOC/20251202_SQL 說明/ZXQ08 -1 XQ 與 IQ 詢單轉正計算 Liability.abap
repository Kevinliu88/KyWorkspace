...
select  a.VTWEG,
        ...
        (select sum(c.MENGE) from ZTCX0001 c
         where a.VTWEG = c.VTWEG and a.zpno = c.idnrk
           and (c.ERDAT between '20250601' and '20251130')
           and c.VBELN <> ''
         group by c.VTWEG, c.idnrk) sum_MENGE
from ZVSD0003 a
where a.zqtybalace >0
  and (a.zym < '202511' or (a.zunptyp <> '2' and a.zym in ('202511','202511')))
  and a.VTWEG in ('C1','B1','J1','G1','N1','T1')
  and a.ZTUNYN = 'X'
...
select  a.VTWEG,
        ...
        (select sum(MOQ_CONFIRM) from ZTCX0001 c
         where a.VTWEG = c.VTWEG and a.idnrk = c.idnrk
           and c.ERDAT between '20251101' and '20251130'
           and c.VBELN <> ''
         group by c.VTWEG, c.idnrk) sum_MENGE
from ztcx0001 a
...
where a.ERDAT between '20251101' and '20251130'
  and a.VBELN <> ''
  and a.VTWEG in ('C1','B1','J1','G1','N1','T1')
