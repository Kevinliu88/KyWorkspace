...
*    === 修正版：使用傳統語法避免編譯錯誤 ===*
        DATA: lv_len       TYPE i,
              ...
              lv_best_last TYPE c LENGTH 1,
              lv_base_len  TYPE i,
              ls_tmp       LIKE ls_029.

        CLEAR: lv_base, lv_best, lv_last_char, lv_best_last.
        lv_len = strlen( ls_029-matnr ).

        IF lv_len > 0.
          lv_off = lv_len - 1.
          lv_last_char = ls_029-matnr+lv_off(1).

          " 判斷最後一碼是否為版本字母(A-Z)
          IF lv_last_char CO 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.
            " 有版本字母 → 基礎碼 = 去掉最後一碼
            lv_base = ls_029-matnr+0(lv_off).
            lv_base_len = lv_off.
          ELSE.
            " 無版本字母 → 整個料號就是基礎碼
            lv_base = ls_029-matnr.
            lv_base_len = lv_len.
          ENDIF.
        ENDIF.

        lv_best = ls_029-matnr.

        " 尋找同基礎碼的最高版本
        LOOP AT gt_pp029 INTO ls_tmp.
          lv_len = strlen( ls_tmp-matnr ).

          IF lv_len > 0.
            lv_off = lv_len - 1.
            lv_last_char = ls_tmp-matnr+lv_off(1).

            " 檢查是否為版本字母
            IF lv_last_char CO 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.
              " 檢查基礎碼是否相同
              IF lv_len > lv_base_len AND
                 ls_tmp-matnr+0(lv_base_len) = lv_base.

                " 比較版本字母
                lv_len_best = strlen( lv_best ).
                lv_off_best = lv_len_best - 1.
                lv_best_last = lv_best+lv_off_best(1).

                IF lv_last_char > lv_best_last.
                  lv_best = ls_tmp-matnr.
                ENDIF.
              ENDIF.
            ELSE.
              " 無版本字母,檢查是否與基礎碼完全相同
              IF ls_tmp-matnr = lv_base.
                " 優先保留無版本字母的基礎料號
                lv_len_best = strlen( lv_best ).
                IF lv_len_best > 0.
                  lv_off_best = lv_len_best - 1.
                  lv_best_last = lv_best+lv_off_best(1).
                  IF lv_best_last CO 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.
                    " 當前 best 有版本字母,改用基礎版本
                    lv_best = ls_tmp-matnr.
                  ENDIF.
                ENDIF.
              ENDIF.
